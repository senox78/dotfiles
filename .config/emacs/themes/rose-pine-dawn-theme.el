...
(autothemer-deftheme
 ...
 ((((class color) (min-colors #xFFFFFF)))                        ;; GUI mode
  ...
  ;; Keep secondary text muted without washing it out against Dawn's light
  ;; background.  The old value was a Moon-style lavender with very little
  ;; contrast on `base'.
  (faded      "#797593")  ;; muted
 ...
 ((default                   (:background base :foreground text))
  ...
  ;; Structural
  ;; Emphasis should not replace the semantic foreground of the surrounding
  ;; face.  This is particularly important for Org headings and links.
  (bold                      (:weight 'bold))
  (italic                    (:slant 'italic))
  ...
  (line-number                               (:foreground faded))
  (line-number-current-line                  (:foreground text :weight 'bold))
...
;; Autothemer 0.2 does not reliably register faces nested under
;; `with-eval-after-load'.  Register the high-impact Org state faces directly
;; so they do not fall back to Emacs's saturated Red1/Red4 defaults.
(custom-theme-set-faces
 'rose-pine-dawn
 '(org-todo ((t (:foreground "#b4637a" :weight bold))))
 '(org-done ((t (:foreground "#286983" :weight bold))))
 '(org-headline-todo ((t (:foreground unspecified))))
 '(org-headline-done ((t (:foreground "#797593")))))
